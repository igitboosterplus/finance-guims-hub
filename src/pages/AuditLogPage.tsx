...
const PAGE_SIZE = 20;
const AUDIT_RESTORE_WINDOW_MS = 24 * 60 * 60 * 1000;
...
export default function AuditLogPage() {
  ...
  const canRestoreAuditEntriesAfter24h = hasPermission(user, 'canRestoreAuditEntriesAfter24h');
  ...
  const isRestoreLockedByAge = (entry: AuditLogEntry) => {
    if (canRestoreAuditEntriesAfter24h) return false;
    return (Date.now() - new Date(entry.timestamp).getTime()) > AUDIT_RESTORE_WINDOW_MS;
  };
  ...
  const handleRestore = async (entry: AuditLogEntry) => {
    ...
    if (isRestoreLockedByAge(entry)) {
      toast.error("Restauration refusée: entrée d'audit de plus de 24h. Demandez l'autorisation à l'administrateur.");
      return;
    }
                        ...
                        <TableCell>
                          {isRestorableDelete(entry) && canRestoreAuditEntries ? (
                            <Button
                              ...
                              disabled={restoringId === entry.id || restoredDeleteAuditIds.has(entry.id) || isRestoreLockedByAge(entry)}
                              title={isRestoreLockedByAge(entry) ? "Restauration bloquée après 24h (sauf droit spécial)" : "Restaurer"}
                            >
